...
async def build_image(
    ...
    # Create .dockerignore if missing to speed up context transfer
    ...
    image_tag = f"dockerforge-{job_id}"
...
async def run_container(job_id: str) -> tuple[bool, str]:
    image_tag = f"dockerforge-{job_id}"
    container_name = f"dockerforge-run-{job_id}"
...
async def remove_image(job_id: str) -> None:
    image_tag = f"dockerforge-{job_id}"
